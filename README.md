...
###
--> There is a core rule matching. If a recipe needs 4 ingredients and only4 existst in the users pantry no rule will match and recipe wont be shown.
It would match small typo's and so on like 'tomatoes' and 'tamato' so that small mismatches are not causing errors or not showing anyn recipes.
...
-'recipes' --> rwecipe name and the preperation steps
